...
class Mt5Broker(BaseBroker):
    ...
    def get_ticker_info(self, symbol: str) -> Union[IAsset, None]:
        ...
        try:
            ...
            tradeable = (
                False
                if tickerInfo.trade_mode == mt5.SYMBOL_TRADE_MODE_DISABLED
                or (tickerInfo.trade_mode == mt5.SYMBOL_TRADE_MODE_CLOSEONLY)
                else True
            )
            shortable = (
                False
                if not tradeable
                or (tickerInfo.trade_mode == mt5.SYMBOL_TRADE_MODE_LONGONLY)
                else True
            )
    ...
    def get_positions(self) -> dict[str, IPosition]:
        try:
            ...
            positions = {
                position.ticket: self.format_position(position)
    ...
    def get_history(self, asset: IAsset, start, end, resolution) -> pd.DataFrame:
        ...
        try:
            ...
            rates = mt5.copy_rates_range(
                asset["symbol"],
                int(resolution),
                start.astimezone(self.TIMEZONE).replace(tzinfo=timezone.utc),
                end.astimezone(self.TIMEZONE).replace(tzinfo=timezone.utc),
            )
            # rates = mt5.copy_rates_range(
    ...
    def execute_insight_order(self, insight, asset) -> Union[IOrder, None]:
        ...
        if insight.type == IOrderType.LIMIT:
            ...
            request["price"] = (
                mt5.symbol_info_tick(asset["symbol"]).ask
                if insight.side == IOrderSide.BUY
                else mt5.symbol_info_tick(asset["symbol"]).bid
            )

        # TODO: insight.type == IOrderType.STOP
        # TODO: insight.type == IOrderType.STOP_LIMIT
    ...
    def format_on_bar(
        self, bar: Any, symbol: Optional[str] = None
    ) -> Optional[pd.DataFrame]:
        if isinstance(bar, np.ndarray):
            index = pd.MultiIndex.from_product(
                [[symbol], pd.to_datetime(bar["time"], utc=True, unit="s")],
    ...
    def format_order(self, order: Any) -> IOrder:
        ...
        res = IOrder(
            ...
            filled_price=order.price_current
            if order.price_open != 0.0
            else (None if orderType != IOrderType.MARKET else order.price_current),
            ...
            else IOrderClass.OTO,
            time_in_force=ITimeInForce.GTC
            if (order.type_time == mt5.ORDER_TIME_GTC)
            else ITimeInForce.DAY,
            status=order.state,
